...
uuids = sort(ascolumn(arrayfun(@(r) getuuid(), 1:50, 'UniformOutput', false)));

for chan = 1:size(h.filtsigma, 2)
    ...
    if idx_sel+1 > length(idx_tr_all)
        idx_tr_rem = [idx_tr_all(idx_sel); min([idx_tr_all(idx_sel)+72.5*cfg.srate, size(h.filtsigma, 1)])];
    else
        idx_tr_rem = idx_tr_all([idx_sel, idx_sel+1]);
    end
    [~, idx_pk_rem] = max(h.filtsigma(idx_tr_rem(1):idx_tr_rem(2), chan));
    ...
    for j = 1:length(idx_tr_nrem)-1
        [~, idx_pk_nrem(j)] = max(h.filtsigma(idx_tr_nrem(j):idx_tr_nrem(j+1), chan)); %#ok<AGROW>
    ...
    for j = 1:length(idx_tr_nrem)
        amp_nrem = [amp_nrem; h.filtsigma(idx_pk_nrem(j), chan) - h.filtsigma(idx_tr_nrem(j), chan)]; %#ok<AGROW>
        dur_nrem = [dur_nrem; idx_pk_nrem(j) - idx_tr_nrem(j)]; %#ok<AGROW>
    end
    ang_rem = h.ang(idx_rem_onset, chan);
    amp_rem = h.filtsigma(idx_pk_rem, chan) - h.filtsigma(idx_tr_rem, chan);
    dur_rem = idx_pk_rem - idx_tr_rem;
    % Extract the ISF parameters of the prior NREM period
    tmp = table();
    tmp.id = uuids(1:length(idx_tr_nrem), 1);
    ...
    tmp = table();
    tmp.id = uuids(end-length(idx_tr_rem)+1:end, 1);
