function [detection,spindle_times_sec,spindle_recs,RectifiedData] = ferrarelli_spindle_detection(C3,fs,sleep,age)
...
% Cowen - changed the 3rd argument to be compatible with my own data.
% 2016
...
global passband_narrow
global passband_wide

switch age
    case 'mice'
        upper_thresh_ratio = 6; % Rats probably lower noise.
        passband_narrow = [10 15]; % this could be widened.
        passband_wide = [5 18];    
    case 'rats'
        upper_thresh_ratio = 6; % Rats probably lower noise.
        passband_narrow = [10 15]; % this could be widened.
        passband_wide = [9 16];
     case 'children'
        upper_thresh_ratio = 6; % Cowen:in my limited experience - 6 results in way too many false positives, but this is what was stated in the paper..
        passband_narrow = [11 15]; % this could be widened.
        passband_wide = [10 16];
    case 'adults'
        upper_thresh_ratio = 8;
        passband_narrow = [11 15];
        passband_wide = [10 16];
end
...
nremsamples = v_Hyp<=-2; % Use data from stage S2+S3+S4
...
nrem_peaks_index=v_Hyp(envelope_peaks)<=-2; % extract samples that are in NREM stage S2+S3+S4
...
detection(1) = 0; detection(end) = 0; 
spindle_recs(:,1) = find(diff(detection)>0)+1;
spindle_recs(:,2) = find(diff(detection)<0)+1;
spindle_times_sec = spindle_recs/fs;
...
%% Functions
    function out = bandpass_filter_ferrarelli(in,Fs)
        ...
        Wp=passband_narrow/(Fs/2);
        Ws=passband_wide/(Fs/2);
        Rp=3;
        Rs=40;
